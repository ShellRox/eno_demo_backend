...
"""
Functions below are adapted from metaplex.transactions
"""

def safe_merge_signers(sgn1, sgn2):
    return list(set(sgn1 + sgn2))
...
def deploy(api_endpoint, source_account, name, symbol, tx=Transaction(), signers=[]):
    # Initalize Client
    ...
    # List signers
    signers = list(set([source_account, mint_account] + signers))
...
def topup(api_endpoint, sender_account, to, amount=None, tx=Transaction(), signers=[]):
    """
    ...
    # List signers
    signers = list(set([sender_account] + signers))
...
def mint(api_endpoint, source_account, contract_key, dest_key, link, supply=1, tx=Transaction(), signers=[]):
    """
    ...
    token_account = TOKEN_PROGRAM_ID
    # List signers
    signers = list(set([source_account] + signers))
...
def send(api_endpoint, source_account, contract_key, sender_key, dest_key, private_key, tx=Transaction(), signers=[]):
    """
    ...
    signers = list(set([source_account, owner_account] + signers))
    # Find PDA for sender
